...
// Pre-cargamos grupos para selector global (no blocking)
const groups = useGroups();
const groupCtx = useGroupContext();
if (auth.isAuthenticated) { groups.fetch().catch(()=>{}); }
  ...
  <div class="min-h-screen bg-gray-50 text-gray-900">
    <header class="border-b bg-white relative z-50">
      <div class="mx-auto max-w-5xl px-4 py-3 flex items-center gap-4">
        ...
        <!-- Selector de grupo global (desktop) -->
        <div v-if="auth.isAuthenticated" class="hidden md:flex ml-4"><GroupSelector /></div>
        <!-- Desktop nav -->
        <nav v-if="auth.isAuthenticated" class="hidden sm:flex ml-auto items-center gap-2">
          <RouterLink to="/" v-slot="{ isActive }"><span :class="linkBaseClasses(isActive)" :aria-current="isActive ? 'page' : undefined">{{ t('nav.matches') }}</span></RouterLink>
          <RouterLink to="/groups" v-slot="{ isActive }"><span :class="linkBaseClasses(isActive)">{{ t('nav.groups') }}</span></RouterLink>
          <RouterLink to="/players" v-slot="{ isActive }"><span :class="linkBaseClasses(isActive)">{{ t('nav.players') }}</span></RouterLink>
          <button @click="logout" class="ml-4 inline-flex items-center gap-1 rounded-md border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-red-500 transition">{{ t('nav.logout') }}</button>
        </nav>
        <nav v-else class="hidden sm:flex ml-auto items-center gap-2">
          <RouterLink to="/login" v-slot="{ isActive }"><span :class="linkBaseClasses(isActive)">{{ t('nav.login') }}</span></RouterLink>
        </nav>
        <!-- Mobile toggle -->
        <button @click="toggleMobileNav" class="sm:hidden ml-auto inline-flex items-center justify-center h-9 w-9 rounded-md border border-gray-300 text-gray-600 hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500" :aria-expanded="mobileNavOpen" :aria-label="mobileNavOpen ? t('nav.closeMenu') : t('nav.openMenu')">
        ...
        <div v-if="mobileNavOpen" class="sm:hidden border-t bg-white shadow-inner">
          <div class="px-4 py-3 flex flex-col gap-2">
            <template v-if="auth.isAuthenticated">
              <!-- Selector grupo en mobile -->
              <div class="px-1 py-1"><GroupSelector /></div>
              <RouterLink to="/" class="w-full" v-slot="{ isActive }"><span :class="linkBaseClasses(isActive) + ' w-full justify-start'">{{ t('nav.matches') }}</span></RouterLink>
              <RouterLink to="/groups" class="w-full" v-slot="{ isActive }"><span :class="linkBaseClasses(isActive) + ' w-full justify-start'">{{ t('nav.groups') }}</span></RouterLink>
              <RouterLink to="/players" class="w-full" v-slot="{ isActive }"><span :class="linkBaseClasses(isActive) + ' w-full justify-start'">{{ t('nav.players') }}</span></RouterLink>
              <button @click="logout" class="relative inline-flex items-center rounded-md px-3 py-2 text-sm font-medium transition bg-red-50 text-red-600 hover:bg-red-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-red-500">{{ t('nav.logout') }}</button>
            </template>
            <template v-else>
              <RouterLink to="/login" class="w-full" v-slot="{ isActive }"><span :class="linkBaseClasses(isActive) + ' w-full justify-start'">{{ t('nav.login') }}</span></RouterLink>
            </template>
          </div>
        </div>
      </transition>
